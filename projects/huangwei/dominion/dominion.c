...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
	return mineaction ( state, handPos,choice1,choice2);
    ...
    case baron:
        return baronaction(choice1,state,currentPlayer);
    ...
    case minion:
	 return minionaction(choice1, state, currentPlayer, handPos);
    ...
    case tribute:
	return tributeaction(state,handPos);
        

    case ambassador:
      return ambassadoraction(choice1, choice2,state, handPos,currentPayer);
...
int j= state->hand[currentPlayer][trashPos];  //store card we will trash
        ...
        if (state->hand[currentPlayer][trashPos] < copper || state->hand[currentPlayer][trashPos] > gold)
        {
            return -1;
        }
        ...
        gainCard(gaincard, state, 2, currentPlayer);
...
int baronaction(int discard, struct gameState *state, int currentPlayer){
...
int ambassadoraction(int reveal, int cardcount,struct gameState *state, int handPos, int currentPlayer){
 	 int j = 0;		//used to check if player has enough cards to discard
        ...
        if (DEBUG)
            printf("Player %d reveals card number: %d\n", currentPlayer, state->hand[currentPlayer][reveal]);

        //increase supply count for choosen card by amount being discarded
        state->supplyCount[state->hand[currentPlayer][reveal]] += cardcount;
        ...
        for (i = 0; i < state->numPlayers; i++)
        {
            if (i != currentPlayer)
            {
                gainCard(state->hand[currentPlayer][reveal], state, 0, i);
            }
        }
        ...
        for (j = 0; j < cardcount; j++)
        {
            for (i = 0; i < state->handCount[currentPlayer]; i++)
            {
                if (state->hand[currentPlayer][i] == state->hand[currentPlayer][reveal])
                {
